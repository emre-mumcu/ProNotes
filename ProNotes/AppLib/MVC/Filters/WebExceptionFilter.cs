...
namespace ProNotes.AppLib.MVC.Filters
{
    public class WebExceptionFilter : ExceptionFilterAttribute
    {
        ...
        {
            WebException? wex = context.Exception as WebException;
            HttpStatusCode statusCode = (wex?.Response as HttpWebResponse)?.StatusCode ?? GetStatusCode(context.Exception.GetType());
        ...
        private void ReturnContent(ExceptionContext context, HttpStatusCode statusCode)
        {
            ...
            var methodDescriptor = string.Format("{0}.{1}.{2}",
                controllerActionDescriptor?.MethodInfo.ReflectedType?.Namespace ?? string.Empty,
                controllerActionDescriptor?.MethodInfo.ReflectedType?.Name ?? string.Empty,
                controllerActionDescriptor?.MethodInfo.Name ?? string.Empty);
